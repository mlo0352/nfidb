...
- Make the iPad Controls reveal a large, high-contrast bottom-left target, keep an explicitly opened toolbar visible until the user closes it or returns to the canvas, and use the compact scrolling layout at common iPad portrait widths so Touch remains reachable.
- Drive VideoToolbox presentation time and WebRTC RTP timestamps from each frame's monotonic capture time. ScreenCaptureKit may stop emitting an unchanged desktop; the first frame after that idle period now carries the full elapsed media-clock advance instead of making Safari interpret it as extreme network jitter and grow a seconds-deep playout buffer.
...
- Hardened Safari recovery so an unavailable keyframe control channel, a failed/disconnected peer, or a first frame that never arrives can escalate to a bounded video-peer rebuild. Safari frame callbacks remain authoritative even when Safari leaves their optional `mediaTime` unchanged. After initial presentation, fresh independent host capture/encode activity distinguishes a stuck Safari decoder from ScreenCaptureKit's healthy static-screen silence, preventing false recovery loops. **Video → Reset video** provides the same repair manually without disturbing input or pairing.
...
- Added branded macOS menu-bar and Dock/taskbar icons using the same NFiDB artwork as Windows. The Dock icon is applied at runtime as well as declared in the signed app bundle, and clicking the menu-bar icon restores and focuses the host window.
- Added a macOS release regression that pairs a real browser through NFiDB's authenticated WebRTC path, requires decoded video to start and advance, forces a recovery IDR, and fails unless remote input selects the low-latency DataChannel. This prevents a compiling encoder or cosmetic browser test from hiding a packaged no-video/slow-mouse regression.
...
- Fullscreen now dismisses Stats, Files, Video, and Keyboard panels before hiding the toolbar. Stats also has its own persistent close button, so an open diagnostic overlay can never become stranded after the toolbar disappears.
...
- Native macOS crate: 15/15 tests pass on an M1 Pro running macOS 27.0 beta, including a real 1080p VideoToolbox SPS/PPS/IDR profile check and a receiver-IDR latch regression; the full cross-platform application links and headless test-pattern/server smoke passes.
- The final signed H.264/WebRTC candidate negotiated Baseline `42001f` in automated Chrome, presented its first frame in 96.3 ms, decoded 128/128 received frames during the assertion window, accepted a forced recovery IDR, and reported zero RTP loss, decoder drops, or freezes. Mouse input was received over the WebRTC DataChannel. This remains desktop-browser evidence, not a physical Safari result.
- A physical iPad Safari session then presented the real Mac display successfully from the signed app; the attached iPad mouse/trackpad and keyboard also controlled the Mac responsively. Pencil pressure, touch-mode UX, HEVC, files, and the longer diagnostic run remain separate field checks.
