...
- Hardened Safari recovery so an unavailable keyframe control channel, a failed/disconnected peer, an unchanged media-time callback, or a first frame that never arrives can escalate to a bounded video-peer rebuild. After initial presentation, fresh independent host capture/encode activity distinguishes a stuck Safari decoder from ScreenCaptureKit's healthy static-screen silence, preventing false recovery loops. **Video → Reset video** provides the same repair manually without disturbing input or pairing.
