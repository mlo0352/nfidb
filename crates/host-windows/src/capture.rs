    ...
    pub fn start_test_pattern(&self) -> Result<(), String> {
        ...
        *self.encoder_thread.lock() = Some(encoder_thread);
        ...
        thread::Builder::new()
            ...
            .spawn(move || test_pattern_loop(pattern_slot, pattern_metrics, max_fps))
            .map_err(|error| error.to_string())?;
        *self.status.lock() = CaptureStatus {
            running: true,
            source: "Generated latency test pattern (1280×720)".to_owned(),
            ..CaptureStatus::default()
        };
        Ok(())
    }
    ...
    while let Some(frame) = slot.take() {
        ...
        match encoder.encode(&yuv) {
            Ok(bitstream) if bitstream.frame_type() != FrameType::Skip => {
                ...
                metrics.encoded(bytes.len(), started.elapsed().as_micros() as u64);
                let _ = video_tx.send(EncodedVideoFrame {
                    data: Arc::from(bytes),
                    duration: Duration::from_secs_f64(1.0 / f64::from(max_fps.max(1))),
                    width,
                    height,
                    keyframe,
                });
...
fn test_pattern_loop(slot: Arc<LatestFrame>, metrics: Arc<Metrics>, max_fps: u32) {
    let width = 1280_u32;
    let height = 720_u32;
    ...
    while !slot.stopped.load(Ordering::Acquire) {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                bgra[offset] = ((x * 255 / width) as u8).saturating_add((frame_number & 31) as u8);
                bgra[offset + 1] = (y * 255 / height) as u8;
                bgra[offset + 2] = if x.abs_diff(bar) < 8 { 255 } else { 24 };
                bgra[offset + 3] = 255;
            }
        }
