...
The PIN and QR remain stable while a browser is paired. An ordinary disconnect starts a fresh ten-minute grace period with the same displayed credentials so a video or page reconnect cannot make the code stale. If you need to invalidate the current browser, use **Session → Reset PIN + QR**; that deliberately releases injected contacts, closes the peer connection, and redraws both credentials. An expired unpaired code also rotates automatically while the desktop app is focused.
...
The live iPad panel separates network RTT/bandwidth, decoder and presentation rate, RTP loss, jitter-buffer/decode cost, frame gaps, startup-to-first-frame, estimated pipeline delay, input continuity, pressure/tilt, mouse/wheel/key/text/gesture counters, and browser/host buffering. It also shows the transport-selected ICE path (iPad and host addresses/ports, candidate type, protocol, network type, and VPN flag when Safari exposes them), per-interval packet loss, and NACK/PLI/FIR recovery activity. If the first frame never arrives, recovery starts unconditionally. After playback begins, fresh host capture/encode activity distinguishes a stuck Safari decoder from ScreenCaptureKit's normal silence on an unchanged desktop; an active stalled stream gets bounded clean-keyframe requests and then a video-only peer rebuild. Pairing and input remain active. **Video → Reset video** triggers that rebuild immediately when manual recovery is useful. Safari supplies exact capture-to-presentation timing only when its WebRTC frame metadata exposes it; otherwise NFiDB labels and uses a component estimate. Exact Pencil-contact-to-host-photon latency still requires a synchronized high-speed camera.
...
The macOS app also installs the branded NFiDB menu-bar status icon. Click it to restore and focus the host window if it is hidden or minimized.
