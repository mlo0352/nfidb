...
| Safari post-start stall recovery | Active host plus unchanged media time, unavailable keyframe channel, or failed peer escalates from bounded IDR requests to a video-only peer rebuild; static Mac screen remains stable | AUTOMATED PASS / FIELD RUN PENDING | Browser unit tests prove unchanged callbacks do not mask a stall, reconnect does not depend on successfully sending an IDR request, and 30 seconds of zero host capture/encode activity causes no request or rebuild; repeated physical recovery remains a release gate |
