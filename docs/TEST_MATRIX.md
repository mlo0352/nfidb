...
| macOS build/tests | Apple-silicon compile, 15 native host tests, cross-platform app link, package signature, app identity | PASS | M1 Pro: 15/15 host tests; package extracted, signature verified, 1024 px transparent ICNS declared in Info.plist, executable reports its version |
...
| macOS packaged video/DataChannel smoke | Pair through the real HTTP/WebRTC path, require first decoded frame and advancing video, then verify input selected the DataChannel | PASS REQUIRED | `scripts/macos-video-smoke.sh` runs the optimized candidate with a generated source and drives installed Chrome; a missing video stream or WebSocket-only input fails the release check |
| macOS app identity | Bundle metadata, high-resolution alpha source, generated ICNS, runtime Dock icon, and menu-bar icon | PASS REQUIRED | `scripts/build-macos.sh` rejects a missing/low-resolution/opaque logo or missing `CFBundleIconFile`; the host applies the same branded asset through eframe/AppKit so the running Dock icon cannot fall back to the generic executable icon |
...
| Safari post-start stall recovery | Active host without presentation callbacks, unavailable keyframe channel, or failed peer escalates from bounded IDR requests to a video-only peer rebuild; static Mac screen remains stable | AUTOMATED PASS / FIELD RUN PENDING | Browser unit tests preserve Safari callbacks whose optional media time does not advance, prove reconnect does not depend on successfully sending an IDR request, and prove 30 seconds of zero host capture/encode activity causes no request or rebuild; repeated physical recovery remains a release gate |
