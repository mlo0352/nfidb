...
| Windows GUI startup | Encoder discovery preserves the caller's COM apartment; release EXE creates its titled window and serves status from its actual bound port | PASS | Apartment regression unit test plus exact-archive `scripts/gui-smoke.ps1`; forced 49121 collision correctly discovered fallback 49122; published v0.6.1 is a required negative fixture |
