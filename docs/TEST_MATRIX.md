...
| macOS real monitor capture | ScreenCaptureKit IOSurface → VideoToolbox → WebRTC | PASS (host path) | Physical M1 Pro run captured and hardware-H.264 encoded 557/557 real 1920×1080 frames in 15 seconds with zero newest-frame replacements or transport drops. The candidate ignores ScreenCaptureKit's normal status-only shutdown sample. Physical iPad presentation remains the end-to-end gate. |
| macOS Quartz remote input | Pencil pressure/tilt/twist, mouse/buttons/wheel, keyboard/text, Command+Tab, Mission Control, minimize | PARTIAL | Accessibility is physically approved; event construction, key maps, modifiers, state release, and semantic commands are automated. Real iPad and drawing-app behavior remain. |
