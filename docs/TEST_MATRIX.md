...
| Windows GUI startup | Encoder discovery preserves the caller's COM apartment; release EXE creates a window and serves status | PASS | Apartment regression unit test plus exact-archive `scripts/gui-smoke.ps1`; published v0.6.1 is a required negative fixture |
