...
| macOS permission/onboarding | First-run defaults, live Screen Recording/Accessibility checks, request/deep-link controls, missing-app recovery | PASS / ACCESSIBILITY PENDING | Packaged NFiDB was granted Screen & System Audio Recording and restarted; ScreenCaptureKit reports display recording allowed and starts the real 1920×1080 stream. Accessibility still needs its physical toggle. |
| macOS real monitor capture | ScreenCaptureKit IOSurface → VideoToolbox → WebRTC | PARTIAL | Packaged NFiDB now opens the real 1920×1080 ScreenCaptureKit stream and initializes VideoToolbox after approval; physical iPad presentation is the remaining end-to-end gate. |
