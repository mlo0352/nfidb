...
An untested receiver starts Auto on hardware H.264 (or OpenH264 when hardware is unavailable). Reported HEVC/AV1 capability makes those modes testable, not preferred; either codec can become Auto-selected only after a successful presentation benchmark. H.264 SDP constraints are backend-aware: VideoToolbox Baseline uses `4200`, while the established Windows paths retain Constrained Baseline. The RTP level remains asymmetric, but the profile and constraint bytes match the encoder as RFC 6184 requires.
