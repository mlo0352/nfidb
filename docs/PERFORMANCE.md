...
After a physical iPad exposed a roughly 30-second first-picture delay, the startup regression was strengthened to join an encoder already emitting delta frames. The final v0.3.0 candidate run rejected one pre-IDR delta frame, received its connection-requested IDR in 63.856 ms, and rendered the first browser frame in 92.4 ms. The automated release gate requires a first browser frame within five seconds and an IDR within 2.5 seconds; the physical iPad rerun remains required before claiming those exact timings on Safari hardware.

The same 11.8-second v0.3.0 candidate run retained 11 synchronized diagnostic samples over 10.002 seconds with none discarded. On its low-motion 1080p Balanced pattern, p95 results were 1.999 ms LAN RTT, 0.179 Mbps receive rate, 32.98 decode FPS, 32.93 presentation FPS, 12.014 ms jitter-buffer residence per frame, 1.481 ms decode cost per frame, 32.366 ms host encode cost, and 92.814 ms component-estimated pipeline delay. Edge did not expose capture-time frame metadata in that run. It delivered 2,402/2,402 simultaneous input samples; native injection averaged 0.001 ms and peaked at 0.004 ms. The clock-corrected host-arrival estimate was below the one-millisecond timer resolution for nearly all samples and peaked at 0.145 ms. RTP loss, decoder drops, freezes, input gaps, integrity mismatches, and transport skips were all zero.
