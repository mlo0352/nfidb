...
The first physical fullscreen-video stress run exposed two burst sources not represented by the host-only benchmark. The test Mac was associated over 2.4 GHz 802.11n at a 144 Mbps PHY rate while Balanced H.264 transmitted roughly 8.5–9 Mbps. NFiDB also restarted the VideoToolbox session every five wall-clock seconds even though the session already had a five-second frame-count IDR interval. The periodic restart was removed; explicit receiver join/recovery still rebuilds for a reliable IDR, and normal active video uses VideoToolbox's own interval. A one-second hard data-rate ceiling at 125% of the selected average bitrate now bounds regional overshoot without creating a frame queue. These changes require a repeated physical Safari run before claiming an end-to-end improvement.
